...
//!
//! ## 装载管线（2026-08-30 用户指令：键位/主题/字体全部走 ghostty 配置）
//!
//! [`load_pipeline`]：
//!
...
//!    v1 theme.rs 的 One Dark Pro 钉值（bg/fg/cursor/selection/ANSI16）
//!    以显式 ghostty 色键装载在默认文件**之前**——用户显式色键后载覆盖；
...
//!
//! C API 只能从文件装载（无 config_set），程序化注入走生成文件（q0 审计
//! #5 实测路径）。层文件写在 `{{tmp}}/ninja-embed-{pid}/`，取证可见。
//!
//! ## 主题资源
//!
//! 具名主题只从 `<XDG>/ghostty/themes/` 或 `<resources>/themes/` 解析；
//! 嵌入构建的资源目录 = `GHOSTTY_RESOURCES_DIR`（resourcesdir.zig，
//! ReleaseFast 生效）。vendored 补丁 0002 把钉版 iterm2_themes 装到
//! `vendor/ghostty/out/share/ghostty/themes`，宿主在 `ghostty_init` 前
//! 设 `GHOSTTY_RESOURCES_DIR` 指过去（[`ensure_resources_dir`]，
//! build.rs 烘焙路径，已设的环境变量不动）。
//!
//! ## ninja.toml 收缩（v2 宿主/插件特有）
//!
//! v1 的 shell/font-family/font-size/[theme] 终端项与 [keys] 键位段不再
//! 属于宿主：终端项走 ghostty 配置，[keys] 语义在 ninja-embed 不复活
//! （2026-08-30 指令）。q2 schema 只收 `[plugins]`（enabled/paths），
//! 只解析不拉起（监督器是 q3；空载零插件进程/零 socket 不变）。出现
//! 收缩掉的键一律 stderr 警告并忽略（[`parse_host_config`]）。
//!
//! ## 热重载
//!
...
// ---------------------------------------------------------------------------
// ODP 钉值（v1 crates/ninja/src/theme.rs 同源，注明来源键；q4 v1 退役不受牵连）
// ---------------------------------------------------------------------------
...
pub fn odp_layer_text() -> String {
    let mut s = String::from("# ninja-embed ODP default theme layer (generated)\n");
    s.push_str("# One Dark Pro 钉值，来源 v1 crates/ninja/src/theme.rs（官方主题源）。\n");
    s.push_str("# 装载在用户默认文件之前：用户显式色键/theme= 覆盖这里。\n");
...
pub fn host_layer_text() -> String {
    let mut s = String::from("# ninja-embed host layer (generated)\n");
    s.push_str("# 宿主动作经 ghostty keybind 系统统一重绑（2026-08-30 指令）：\n");
    s.push_str("# 插件面板认领空闲动作 toggle_visibility；ghostty 动作集封闭，\n");
    s.push_str("# 自定义动作名不可用（Binding.zig InvalidAction）。\n");
...
/// v2 `[plugins]`：只解析不拉起（监督器 q3；空载零插件进程/零 socket）。
...
/// ninja.toml 路径（v1 同款：NINJA_CONFIG 覆盖 → ~/.config/ninja/ninja.toml）。
...
/// 一律 stderr 警告并忽略——不整体降级（与 v1 deny_unknown_fields 全降级
/// 不同：收缩是常态，用户旧文件应尽量保留可用部分）。
pub fn parse_host_config(text: &str) -> HostConfig {
    let mut cfg = HostConfig::default();
    let value: toml::Value = match toml::from_str(text) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("ninja-embed: ninja.toml 解析失败（忽略全部内容）: {e}");
            return cfg;
        }
    };
    let Some(table) = value.as_table() else {
        eprintln!("ninja-embed: ninja.toml 顶层不是表（忽略全部内容）");
        return cfg;
    };
    for (key, val) in table {
        match key.as_str() {
            ...
            "keys" => eprintln!(
                "ninja-embed: ninja.toml [keys] 已收缩（2026-08-30 指令：键位全量继承 \
                 ghostty keybind 系统），忽略——重绑请写 ghostty 配置 \
                 `keybind = super+x=action`（config 路径见启动日志）"
            ),
            k if V1_TERMINAL_KEYS.contains(&k) => eprintln!(
                "ninja-embed: ninja.toml `{k}` 已收缩——终端配置（shell/字体/主题色）\
                 走 ghostty 配置文件，忽略"
            ),
            other => eprintln!(
                "ninja-embed: ninja.toml 未知键 `{other}`（v2 只收 [plugins]），忽略"
            ),
        }
    }
    cfg
...
fn parse_plugins(val: &toml::Value, cfg: &mut HostConfig) {
    let Some(t) = val.as_table() else {
        eprintln!("ninja-embed: ninja.toml [plugins] 不是表，忽略");
        return;
    };
    for (key, v) in t {
        match key.as_str() {
            "enabled" => {
                let Some(arr) = v.as_array() else {
                    eprintln!("ninja-embed: [plugins] enabled 不是数组，忽略");
                    continue;
                };
                ...
                for item in arr {
                    match item.as_str() {
                        ...
                        None => eprintln!("ninja-embed: [plugins] enabled 项不是字符串，忽略"),
                    }
                }
            }
            "paths" => {
                let Some(pt) = v.as_table() else {
                    eprintln!("ninja-embed: [plugins] paths 不是表，忽略");
                    continue;
                };
                for (name, pv) in pt {
                    match pv.as_str() {
                        ...
                        _ => eprintln!("ninja-embed: [plugins.paths] {name} 不是非空字符串，忽略"),
                    }
                }
            }
            other => eprintln!("ninja-embed: ninja.toml [plugins] 未知键 `{other}`，忽略"),
        }
    }
}
...
pub fn load_host_config() -> HostConfig {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(e) => {
            eprintln!("ninja-embed: 读 {path:?} 失败（{e}），用默认值");
            HostConfig::default()
        }
    }
}
...
fn layer_dir() -> PathBuf {
    ...
    d.push(format!("ninja-embed-{}", std::process::id()));
    d
}
...
pub fn ensure_resources_dir() {
    ...
    // SAFETY: main 线程早期、ghostty_init 之前（唯一入口 main.rs 调）。
...
unsafe fn print_diagnostics(cfg: ghostty_config_t) -> u32 {
    ...
    for i in 0..n {
        ...
        if !d.message.is_null() {
            ...
            eprintln!("ninja-embed: ghostty 配置诊断: {msg}");
        }
    }
    n
}
...
pub struct KeyEquivalent {
    /// keyEquivalent 字符（箭头等功能键用 F700 系字符，v1 同惯例）。
    pub key: u16,
    pub cmd: bool,
    pub ctrl: bool,
    pub alt: bool,
...
pub fn dump_effective_config(
    ...
    s.push_str(&format!(
        ...
        get_color(cfg, "selection-background").map(json_rgb).unwrap_or_else(|| "null".into())
    ));
    s.push_str(&format!(
        "  \"font_size\": {},\n",
        get_f32(cfg, "font-size").map(|v| v.to_string()).unwrap_or_else(|| "null".into())
    ));
    s.push_str(&format!(
        "  \"palette16\": {},\n",
        get_palette16(cfg)
    ...
    if let Err(e) = std::fs::write(path, s) {
        eprintln!("ninja-embed: 写 NINJA_CFG_DUMP {path:?} 失败: {e}");
    }
}
...
"##,
    ...
    fn host_config_default_empty_no_spawn() {
        ...
        assert_eq!(parse_host_config("[plugins]\nenabled = []").plugins.enabled, Vec::<String>::new());
        // 破损 TOML：全忽略（不炸）。
