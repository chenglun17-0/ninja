...
/// 焦点环颜色源链：cursor-color → foreground → ODP 光标蓝（主题系统 q2：
/// v1 静态 #528bff 变为跟随生效配置；无任何配置时的末位兑底仍是 ODP 钉值。
/// 注：cursor-color 是 ?TerminalColor 联合、无 cval——C API 读不出
///（config_get 恒 false），实际链生效段是 foreground → 钉值。
...
pub fn init(app: ghostty_app_t, config: ghostty_config_t, load_info: crate::config::LoadInfo) {
    ...
    if !host_config.plugins.enabled.is_empty() {
        ...
        eprintln!(
            "ninja-embed: ninja.toml 启用插件 {:?}（q2 仅解析，监督器在 q3 拉起）",
...
pub fn schedule_reload(reason: &str) {
    ...
    if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
        eprintln!("ninja-embed: 热重载排期（{reason}）");
    }
...
pub fn reload_tick() {
    ...
    eprintln!("ninja-embed: 配置已重载（用户 theme={} ODP={} 监视 {} 文件）",
...
pub fn close_leaf_deferred(view: &SurfaceHostView) {
    if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
        eprintln!("ninja-embed: close_leaf_deferred pane={}", view.pane_id());
    }
...
unsafe fn dispatch_action(
    ...
    if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
        eprintln!("ninja-embed: action tag={} has_view={}", action.tag as i32, view.is_some());
    }
    unsafe {
        match action.tag {
            // ---- 窗/tab/split 布局树 ----
            ...
            GHOSTTY_ACTION_CONFIG_CHANGE => {
                if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
                    eprintln!("ninja-embed: CONFIG_CHANGE（新配置已传播全部 surface）");
                }
                true
            }
            ...
            GHOSTTY_ACTION_OPEN_CONFIG => {
                eprintln!(
                    "ninja-embed: open_config：编辑 ghostty 配置文件（路径见启动日志/用户配置目录）"
                );
                true
            }
...
pub fn current_surface_view() -> Option<Retained<SurfaceHostView>> {
    ...
    // 兜底：无 key/main 窗口（后台启动未成 key 等）时取任一活面——
...
pub unsafe extern "C" fn close_surface_cb(userdata: *mut c_void, process_alive: bool) {
    ...
    if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
        eprintln!("ninja-embed: close_surface_cb alive={} process_alive={process_alive}", view.surface_opt().is_some());
    }
