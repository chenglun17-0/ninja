//! q0 取证模式（保留可重跑）：自驱动的 libghostty 能力审计时间线
//! （网格读取、hyperlink、屏幕快照、surface 之上合成层、配置运行时改、
//! 键位拦截），证据写进 --evidence-dir（日志 + 截图 + 报告）。
//! 结论与复现见 docs/Q0-CAPABILITY-AUDIT.md。q1 起默认模式是交互壳
//!（[`crate::app`]）；本模块逻辑与 q0 提交一字未改，只收编为模块。
...
unsafe extern "C" fn wakeup_cb(_userdata: *mut c_void) {
    ...
    // 可能从 IO 线程调用：唤醒主 RunLoop，主线程 timer 里做 app_tick。
    ...
    unsafe {
        match action.tag {
            ...
            GHOSTTY_ACTION_OPEN_URL => {
                ...
                let url = String::from_utf8_lossy(url).to_string();
                log_line(d, &format!("action OPEN_URL kind={:?} url={url}", action.action.open_url.kind));
                d.open_url = Some(url);
                true
            }
...
unsafe extern "C-unwind" fn timer_cb(_timer: *mut objc2_core_foundation::CFRunLoopTimer, _info: *mut c_void) {
    let d = d();
    ...
    if d.step == 0 && t > 0.4 {
        ...
        log_line(
            ...
            &format!(
                ...
                size.width_px, size.height_px, size.columns, size.rows, size.cell_width_px, size.cell_height_px
            ),
        );
    ...
    if d.step == 2 && t > 1.8 {
        d.step = 3;
        let size = unsafe { ghostty_surface_size(d.surface) };
        let read_all = unsafe { read_text(d.surface, 0, 0, (size.columns - 1) as u32, (size.rows - 1) as u32) };
        let has_grid =
    ...
    if d.step == 3 && t > 2.3 {
        d.step = 4;
        let size = unsafe { ghostty_surface_size(d.surface) };
        let full = unsafe { read_text(d.surface, 0, 0, (size.columns - 1) as u32, (size.rows - 1) as u32) };
        ...
        log_line(
            d,
            &format!("read_text(region rows {r1}..{}) = {region:?}", r1 + 1),
        );
        check(
            d,
            "grid-read-region",
    ...
    if d.step == 4 && t > 2.8 {
        ...
        for row in 0..rows {
            for col in [3.5, 20.0, 40.0, 55.0] {
                ...
                if let Some(u) = &d.over_link_url {
                    ...
                    log_line(
                        d,
                        &format!("mouse_pos(view pt, mods=⌘) row {row} col {col:.0} -> MOUSE_OVER_LINK 触发"),
                    );
                    break;
                }
            }
        ...
        if got.is_none() {
            log_line(d, &format!("mouse_pos 全网格扫描（{} 行×4 列, mods=⌘）均未触发 MOUSE_OVER_LINK", rows));
        }
    }
    ...
    if d.step == 12 && t > 7.2 {
        ...
        log_line(
            d,
            &format!("report written: {}/report.txt", d.evidence_dir.display()),
        );
...
fn rl_remove_timer(rl: &objc2_core_foundation::CFRunLoop, timer: &objc2_core_foundation::CFRunLoopTimer) {
...
pub fn run(evidence_dir: PathBuf) {
    ...
    unsafe {
        ...
        // 3. AppKit 壳：窗口 + 宿主自建 NSView（不碰 v1 引擎）
        let mtm = mtm();
        ...
        let rl = objc2_core_foundation::CFRunLoop::main().unwrap(); // unused now
        ...
        let timer =
            objc2_core_foundation::CFRunLoopTimer::new(None, 0.05, 0.016, 0, 0, Some(timer_cb), &mut ctx)
                .expect("timer create");
        rl_add_timer(&rl, timer.as_ref());

        // 7. 主事件循环：NSApp.run（AppKit 会在其中跑主 RunLoop；
        //    stop_app_runloop 用 NSApp.stop + 哑事件让它返回）。
        app.run();
