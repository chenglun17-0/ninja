...
pub struct Ivars {
    ...
    /// 插件面板（ninjaPlugins: 创建/复用；关窗只藏不毁）。
...
define_class!(
    ...
    unsafe impl NSApplicationDelegate for AppDelegate {
        ...
        fn applicationShouldTerminateAfterLastWindowClosed(
            ...
            if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
                eprintln!("ninja-embed: terminateAfterLastWindowClosed? -> true");
            }
            true
        }
    ...
    unsafe impl NSWindowDelegate for AppDelegate {
        ...
        fn window_should_close(&self, sender: &NSWindow) -> bool {
            ...
            if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
                eprintln!("ninja-embed: windowShouldClose -> {ok}");
            }
            ok
        }
        ...
        fn windowWillClose(&self, notification: &NSNotification) {
            if std::env::var_os("NINJA_Q1_DEBUG").is_some() {
                eprintln!("ninja-embed: windowWillClose");
            }
            ...
            for step in seq.split(',').map(str::trim) {
                match step {
                    ...
                    "closebinding" => {
                        ...
                        let Some(s) = f else {
                            eprintln!("ninja-embed: NINJA_P2_SELFTEST closebinding 无可用 surface");
                            continue;
                        };
        ...
        fn ninja_zoom_tick(&self, _timer: Option<&objc2::runtime::AnyObject>) {
            ...
            *self.ivars().zoom_last.borrow_mut() = Some(content.clone());
            ...
            let Some(w) = window else {
                eprintln!("ninja-embed: NINJA_ZOOM_FILE 无窗口可 zoom");
                return;
            };
            let Some(container) = crate::pane::container_of(&w) else {
                eprintln!("ninja-embed: NINJA_ZOOM_FILE 窗口内容不是 PaneContainer");
                return;
            };
            match content.as_str() {
                ...
                s if s.starts_with("dump") => {
                    let Some(out) = std::env::var_os("NINJA_ZOOM_DUMP") else {
                        eprintln!("ninja-embed: NINJA_ZOOM_FILE=dump 需要 NINJA_ZOOM_DUMP 输出路径");
                        return;
                    };
                    let _ = std::fs::write(&out, container.zoom_state_json());
                }
                other => eprintln!(
                    "ninja-embed: NINJA_ZOOM_FILE 动作 {other:?} 无效（toggle | zoom | unzoom | split | dump）"
                ),
            }
        }
