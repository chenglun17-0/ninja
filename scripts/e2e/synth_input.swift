// synth_input.swift —— E2E 真实输入驱动（键盘 PostToPid + 鼠标全局 HID tap）
//
...
//
// 编译（产物不入库，quality-gates.sh 运行时自编译）：
...
func postMouse(_ type: CGEventType, posX: Double, posY: Double, flags: CGEventFlags) {
    let src = CGEventSource(stateID: .hidSystemState)
    guard let ev = CGEvent(mouseEventSource: src, mouseType: type,
                           mouseCursorPosition: CGPoint(x: posX, y: posY), mouseButton: .left)
    else { FileHandle.standardError.write("mouse event create failed\n".data(using: .utf8)!); exit(1) }
    ev.flags = flags.union(CGEventFlags(rawValue: 0x100))
    ev.post(tap: .cghidEventTap)
}

func postClick(pid: pid_t, posX: Double, posY: Double, flags: CGEventFlags) {
    _ = pid
    postMouse(.mouseMoved, posX: posX, posY: posY, flags: flags)
    usleep(150_000)
    postMouse(.leftMouseDown, posX: posX, posY: posY, flags: flags)
    usleep(80_000)
    postMouse(.leftMouseUp, posX: posX, posY: posY, flags: flags)
    usleep(120_000)
}
...
case "key":
    ...
    if args.count >= 5 {
        for modName in args[4].split(separator: ",") {
            switch modName {
...
case "click":
    guard args.count >= 5, let pid = pid_t(args[2]), let posX = Double(args[3]), let posY = Double(args[4]) else { exit(2) }
    var flags: CGEventFlags = []
    if args.count >= 6 {
        for modName in args[5].split(separator: ",") {
            switch modName {
            case "cmd": flags = flags.union(cmdFlag)
            case "shift": flags = flags.union(shiftFlag)
            case "alt": flags = flags.union(altFlag)
    ...
    postClick(pid: pid, posX: posX, posY: posY, flags: flags)
case "wins":
    ...
    var out: [[String: Any]] = []
    for winInfo in list where (winInfo[kCGWindowOwnerPID as String] as? Int) == Int(pid) {
        let bounds = winInfo[kCGWindowBounds as String] as? [String: Int] ?? [:]
        out.append([
            "id": winInfo[kCGWindowNumber as String] as? Int ?? 0,
            "layer": winInfo[kCGWindowLayer as String] as? Int ?? 0,
            "title": winInfo[kCGWindowName as String] as? String ?? "",
            "bounds": [
    ...
    do {
        ...
        FileHandle.standardError.write("wins serialize failed\n".data(using: .utf8)!)
        exit(1)
    }
default:
