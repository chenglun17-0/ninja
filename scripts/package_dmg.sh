...
[[ -d "$APP" ]] || {
  echo "错误：$APP 不存在，先跑 scripts/package_app.sh" >&2
  exit 1
}
...
[[ -f "$MNT/Ninja.app/Contents/MacOS/ninja" ]] || {
...
[[ -f "$MNT/Ninja.app/Contents/Resources/AppIcon.icns" ]] || {
  echo "错误：DMG 卷内缺 Resources/AppIcon.icns" >&2
  exit 1
}
[[ "$(defaults read "$MNT/Ninja.app/Contents/Info" CFBundleIconFile)" == "AppIcon.icns" ]] || {
  echo "错误：DMG 卷内 Info.plist CFBundleIconFile ≠ AppIcon.icns" >&2
  exit 1
}
# 主题资源随包断言（q4）：分发机上具名主题解析的唯一资源源。
DMG_THEMES="$(find "$MNT/Ninja.app/Contents/Resources/ghostty/themes" -type f | wc -l | tr -d ' ')"
[[ "$DMG_THEMES" -gt 500 ]] || {
  echo "错误：DMG 卷内主题资源异常少（${DMG_THEMES}）" >&2
  exit 1
}
echo "    DMG 内 themes：${DMG_THEMES} 个文件"
[[ -f "$MNT/Ninja.app/Contents/Resources/terminfo/78/xterm-ghostty" ]] || {
  echo "错误：DMG 卷内缺 terminfo/78/xterm-ghostty" >&2
  exit 1
}
cleanup_dmg
trap - EXIT
