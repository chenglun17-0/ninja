...
[[ $(find "$ICONSET" -name '*.png' | wc -l | tr -d ' ') -eq 10 ]] || {
  echo "错误：iconset 应 10 个 PNG" >&2
  exit 1
}
for i in "${!NAMES[@]}"; do
  f="$ICONSET/${NAMES[$i]}.png"
  want=${PX[$i]}
  got="$(sips -g pixelWidth -g pixelHeight "$f" 2>/dev/null |
    awk '/pixel/{print $2}' | paste -sd, -)"
  [[ "$got" == "$want,$want" ]] || {
    echo "错误：${NAMES[$i]}.png 期望 ${want}×${want}，实际 $got" >&2
    exit 1
  }
done
...
for pt in "6 6" "1018 6" "6 1018" "1018 1018"; do
  ...
  [[ "$got" == *"a=0" ]] || {
    echo "错误：1024 图 ($1,$2) 圆角外应透明，实际 $got（蒙版未生效？）" >&2
    exit 1
  }
done
# 中心必须不透明且非全黑（内容画上了）
...
[[ "$got" == *"a=0" ]] || {
  echo "错误：32px 角不透明（$got）" >&2
  exit 1
}
got="$(swift "$SWIFT_ICON" --sample "$SMALL" 16 16)"
[[ "$got" == *"a=255" ]] || {
  echo "错误：32px 中心无内容（$got）" >&2
  exit 1
}
...
[[ $(find "$RT" -name '*.png' | wc -l | tr -d ' ') -eq 10 ]] || {
  echo "错误：icns 回读 iconset 不是 10 个 PNG（坏 icns）" >&2
  exit 1
}
rm -rf "$(dirname "$RT")"
